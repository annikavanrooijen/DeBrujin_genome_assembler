class Read:
    ...
    def __init__(self, lines):
        '''
        Initialize a Read object from FASTA file lines.

        The first line is interpreted as the header (without '>'),
        and all remaining lines are concatenated into a single
        nucleotide sequence.

        Args:
            lines (list[str]): Lines belonging to one FASTA entry.
        '''
    ...
    def get_kmers(self, kmersize):
        '''
        Extract all k-mers of a given length from the read sequence
        and count their occurrences.

        Args:
            kmersize (int): Length of the k-mers.
